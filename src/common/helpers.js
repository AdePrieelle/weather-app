...
export const formatLocalDateTimestampWeekday = (timestamp, timezoneOffset) => {
  const timeStampDate = new Date(timestamp*1000).getTime();
  const timeCity = timeStampDate + timezoneOffset*1000;
  const timeStampTimeLocal = new Date(timeCity);
  const day = timeStampTimeLocal.getUTCDay();
...
export const rotateWindArrow = (windDegrees) => {
  ...
  return (
    <div className="wind-arrow-wrapper">
      ...
      <i className="fas fa-location-arrow wind-arrow" style={{rotate: `${(amountToRotate-45)+180}deg`}}></i>
    </div>
  )
}

export const rotateWindArrowBeaufort = (windDegrees, windSpeedBeaufort) => {
  const val = Math.floor((windDegrees / 45) + 0.5);
  const arr = [0, 45, 90, 135, 180, 225, 270, 315];
  const amountToRotate = arr[val % 8];
